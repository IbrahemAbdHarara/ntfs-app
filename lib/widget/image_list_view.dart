...
class ImageListView extends StatefulWidget {
  const ImageListView({Key? key, required this.startInadex, this.duration = 30})
      : super(key: key);

  final int startInadex;
      ...
      child: SizedBox(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            return _ImageTile(
                image: 'images/${widget.startInadex + index}.png');
          },
        ),
      ),
    );
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => NftScreen(image: image),
            ));
      },
    //  tag: image,
      child: Image.asset(
